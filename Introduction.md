...
OpinionMarket is designed with a microservice architecture built using REST APIs. This architecture style improves the application's fault-tolerance, maitainability, and flexibility. However, there is a trade-off involved because it creates challenges related to orchestration, data consistancy, and service discovery. Opinionmarket solves these common problems. Orchestration and data consistancy are handled by using sagas and compensation transactions. Service discovery is performed by leverging the Spring Cloud Eureka tool. 
